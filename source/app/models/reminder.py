...
class Reminder(db.Model):
    ...
    def get_next_occurrences(self, count=5):
        """Return next N occurrences using rrule if recurrence is defined."""
        logger.debug("get_next_occurrences() method called")

        if not self.reminder_rrule or not self.reminder_date_start:
            return []
        rule = rrulestr(self.reminder_rrule, dtstart=self.reminder_date_start)
