...
def get_reminder_next_occurrence(reminder_uuid, reminder_recurrence_type, reminder_recurrence_rrule, reminder_date_end):
    """
    ...
    if reminder_recurrence_type == "NONE":
        ...
        next_occurrence = get_next_occurrences_date_only(reminder_recurrence_rrule, count=1)
...
def send_alert_notification(user_alert_webhook_url, reminder_title, reminder_due_date):
    payload = {
        "text": f"Remindly Alert for Your Reminder : {reminder_title} - Due date {reminder_due_date} is approaching!"
    }
