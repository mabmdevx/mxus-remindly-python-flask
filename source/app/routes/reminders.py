...
def update_reminder(reminder_uuid):
    ...
    if reminder.reminder_recurrence_type != "NONE" and reminder.reminder_recurrence_rrule:
        ...
        next_occurrences = get_next_occurrences_date_only(reminder.reminder_recurrence_rrule, count=5)
...
def api_preview_next_occurrences():
    ...
    next_occurrences = get_next_occurrences_date_only(rrule_str, count=5)
...
def send_alerts():
    ...
    for each_user in users:
        ...
        if each_user.user_alert_webhook_url is None:
            ...
            # Get reminders that belong to that user
            my_reminders = Reminder.query.filter_by(
            ...
            for each_reminder in my_reminders:
                if each_reminder.reminder_display_date_next_occurrence == "N/A":
                    # Skip reminders that don't have a next occurrence
                    logger.debug("Skipping reminder as it doesn't have a next occurrence: %s", each_reminder.reminder_uuid)
                    continue
                else:
                    logger.debug("reminder_uuid: %s", each_reminder.reminder_uuid)
                    logger.debug("reminder_display_date_next_occurrence: %s", each_reminder.reminder_display_date_next_occurrence)
                    logger.debug("reminder_sort_date_next_occurrence: %s", each_reminder.reminder_sort_date_next_occurrence)
                
                    reminder_due_date = each_reminder.reminder_sort_date_next_occurrence
                    logger.debug(f"reminder_due_date: {reminder_due_date}")

                    #logger.debug(type(reminder_due_date), type(today))

                    # Calculate due date diff
                    due_date_diff = reminder_due_date - today
                    logger.debug(f"due_date_diff: {due_date_diff}")
                    # Convert timedelta object to days
                    due_date_diff_days = due_date_diff.days
                    logger.debug(f"due_date_diff_days: {due_date_diff_days}")

                    # Current threshold for sending alerts
                    alert_threshold = 5
                    logger.debug(f"alert_threshold: {alert_threshold}")
                
                    if due_date_diff_days <= alert_threshold and reminder_due_date >= today:
                        # Send alert
                        logger.info(f"Sending alert for reminder: {each_reminder.reminder_uuid} | Due Date Diff Days: {due_date_diff_days} days")

                        response_boolean_flag = send_alert_notification(each_user.user_alert_webhook_url, each_reminder.reminder_title, reminder_due_date)
                        if response_boolean_flag == False:
                            logger.error(f"Error sending notification for reminder: {each_reminder.reminder_uuid}")
                        else:
                            logger.info(f"Notification sent successfully for reminder: {each_reminder.reminder_uuid}")
                    else:
                        logger.debug(f"Skipping reminder as it's not due yet: {each_reminder.reminder_uuid}")
                        continue
